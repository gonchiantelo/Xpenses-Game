    ...
    <div className="page" data-theme={group.palette}>
      <header className="page-header" style={{ borderBottom: `1px solid ${palette.color}30` }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button id="btn-quick-invite" className="btn btn--icon btn--ghost" 
            title="Invitar amigos" onClick={() => router.push(`/groups/${groupId}/settings?action=invite`)}>
            ➕
          </button>
          <Link href={`/groups/${groupId}/settings`} id="btn-group-settings" className="btn btn--icon btn--ghost" title="Configuración y Temas">
            ⚙️
          </Link>
        </div>
      </header>
